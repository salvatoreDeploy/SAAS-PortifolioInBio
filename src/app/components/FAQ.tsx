...
export default function FAQ() {
  ...
  return (
    <div className="my-20 flex flex-col items-center gap-16">
      <h3 className="text-4xl font-bold text-heading">Duvidas frequentes</h3>

      <div className="flex gap-3">
        <div className="flex flex-col gap-3">
          {faqItem.slice(0, Math.ceil(faqItem.length / 2)).map((item) => (
            <FAQItem
              key={item.title}
              title={item.title}
        ...
        <div className="flex flex-col gap-3">
          {faqItem.slice(Math.ceil(faqItem.length / 2)).map((item) => (
            <FAQItem
              key={item.title}
              title={item.title}
              description={item.description}
